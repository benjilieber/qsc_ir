...
%         ismembertol(A.key_rate(:), relative_gap_rate * theoretic_key_rate, 0.04), :);
    ...
    empty_strings = strings(height(A), 1);
    switch code_strategy
        case 'ldpc'
            A.mb_desired_success_rate(:,1) = 0.0;
            A.mb_full_rank_encoding(:) = empty_strings;
        case 'polar'
            A.mb_desired_success_rate(:,1) = 0.0;
            A.mb_full_rank_encoding(:,1) = 0.0;
            A.mb_full_rank_encoding(:,1) = "hi";
%             A.mb_full_rank_encoding(:,1) = repmat({''}, height(A), 1);

%             repmat({''}, nrow, 1)
            unique(A.mb_full_rank_encoding)
    end
end