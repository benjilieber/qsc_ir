...
class LinearCodeGenerator(object):
    ...
    def generate_encoding_matrix(self, num_encoding_columns):
        ...
        encoding_matrix = np.random.choice(range(0 if self.cfg.use_zeroes_in_encoding_matrix else 1, self.cfg.q),
                                           (self.cfg.block_length, num_encoding_columns))
        while util.calculate_rank(encoding_matrix, self.cfg.q) < num_encoding_columns:
            encoding_matrix = np.random.choice(range(0 if self.cfg.use_zeroes_in_encoding_matrix else 1, self.cfg.q),
                                               (self.cfg.block_length, num_encoding_columns))
