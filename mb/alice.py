...
class Alice(object):
    ...
    def encode_blocks(self, encode_new_block, blocks_indices, number_of_encodings, encoding_matrix_prefix,
                      encoding_format):
        """
        Alice encodes A.
        ...
        if encoding_format == LinearCodeFormat.MATRIX:
            encoding_matrices_delta = np.zeros([len(blocks_indices), self.cfg.block_length, number_of_encodings],
                                               dtype=int)
            ...
            random_kernel_element = self.encoder.encode_single_block(kernel_base, np.random.choice(range(self.cfg.q),
                                                                                                   len(kernel_base)))
