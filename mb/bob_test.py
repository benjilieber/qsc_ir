...
class BobTest(unittest.TestCase):
    ...
    def test_single_block(self):
        ...
        [cur_candidates_num, num_candidates_per_block, min_candidate_error] = bob.decode_multi_block([0], np.array(
            [[[0, 1], [1, 1], [2, 0]]]), np.array([2, 0]))
    ...
    def test_multi_block(self):
        ...
        [cur_candidates_num, num_candidates_per_block, min_candidate_error] = bob.decode_multi_block([1], np.array(
            [[[2, 1], [1, 1], [0, 0]]]), np.array([0, 0]))
        ...
        [cur_candidates_num, num_candidates_per_block, min_candidate_error] = bob.decode_multi_block([1, 2], np.array(
            [[[2, 0], [0, 1], [2, 1]], [[0, 1], [2, 1], [0, 2]]]), np.array([2, 1]))
    ...
    def test_multi_block_error(
            self):  # This fails but we're fine with this because the significance of radius has changed (single-block radius instead of multi-block radius)
        bob = Bob(MbCfg(q=3, num_blocks=3, block_length=3, radius=1), [0, 1, 2, 2, 1, 2, 0, 0, 1])

        bob.decode_multi_block([0], np.array([[[0, 1], [1, 1], [2, 0]]]), np.array([2, 0]))
        [cur_candidates_num, num_candidates_per_block, min_candidate_error] = bob.decode_multi_block([1], np.array(
            [[[2, 1], [1, 1], [0, 0]]]), np.array([0, 0]))
        self.assertEqual(bob.a_candidates, [[[1, 2, 0], [0, 0, 0]], [[1, 2, 0], [0, 0, 1]], [[1, 2, 0], [0, 0, 2]],
